...
const Chap3Calculator = () => {
  ...
  const cong_suat_tinh_toan = (P3, k, kz, kn) => {
    let result = Number(P3) * Number(k) * Number(kz) * Number(kn);
    return result;
  }

  const cong_suat_cho_phep = (n01, Pt) => {
    const powerTable = {
  ...
  const khoang_cach_truc = (p) => {
    let result = 40 * Number(p);
    return result;
  }

  const so_mat_xich = (a, p, Z1, Z2) => {
  ...
  const tinh_lai_khoang_cach_truc = (p, xc, Z1, Z2) => {
  ...
  const a = (a_star, del_a) => {
    let result = Number(a_star) - Number(del_a);
    return result;
  }

  const so_lan_va_dap_cua_xich = (Z1, n3, xc) =>{
