...
chap3Routes.route('/Chap3/:id').post(async (request, response) => {
  try {
    ...
    const kz = Chapter3Function.kz(request.body.z01, request.body.z01);

    const kn = Chapter3Function.kn(request.body.n01, Chap2Data.n3);
    ...
    const P = Chapter3Function.cong_suat_cho_phep(request.body.n01, Pt);

    const p = Chapter3Function.buoc_xich(request.body.n01, P);

    const khoang_cach_truc = Chapter3Function.khoang_cach_truc(p);

    const x = Chapter3Function.so_mat_xich(khoang_cach_truc, p, request.body.z01, Z2);

    const a_star = Chapter3Function.tinh_lai_khoang_cach_truc(p, x, request.body.z01, Z2);

    const delta_a = Chapter3Function.delta_a(request.body.Da, a_star);

    const a = Chapter3Function.a(a_star, delta_a);

    const i = Chapter3Function.so_lan_va_dap_cua_xich(request.body.z01, Chap2Data.n3, x);

    const Q = Chapter3Function.tai_trong_pha_hong(p);

    const q = Chapter3Function.khoi_luong_1m_xich(p);

    const v = Chapter3Function.v(request.body.z01, p, Chap2Data.n3);

    const Ft = Chapter3Function.Ft(Chap2Data.P3, v);

    const Fv = Chapter3Function.Fv(q, v);

    const F0 = Chapter3Function.F0(request.body.kf, q, a);

    const s = Chapter3Function.he_so_an_toan(Q, request.body.kd, Ft, F0, Fv);

    const d1 = Chapter3Function.d1(p, request.body.z01);

    const d2 = Chapter3Function.d2(p, Z2);

    const da1 = Chapter3Function.da1 (p, request.body.z01);
    ...
    const oH1 = Chapter3Function.oH1 (request.body.kr1, request.body.kd, Ft, Fvd, A_dt)
